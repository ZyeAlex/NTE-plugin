...
export default async function render(e, p, renderData = {}, cfg = {}) {
  ...
  // copyright
  let copyright = ` 异环 | ${title}  <span class="version">${version}</span> `
  if (cfg.origin) copyright += `| 数据源 <span class="version">${cfg.origin}</span> `
  return e.runtime.render('NTE-plugin', p, renderData, {
    ...cfg,
    beforeRender({ data }) {
      return {
        ...data,
...
export async function saveRender(e, p, url, renderData = {}, ...args) {
  ...
  if (msgRes) {
    ...
    for (const i of message_id) {
      await redis.set(`NTE:original-picture:${i}`, url, { EX: 3600 * 3 })
    }
